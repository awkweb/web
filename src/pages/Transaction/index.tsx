...
class TransactionPage extends React.Component<Props> {
  public async componentDidMount() {
    const {
      ...
      location: { state },
      rootStore: {
        transactionFormStore: { initForm, getBudgets, getTransaction, setId },
          ...
          <Row>
            <Col xs={12}>
              ...
              >
                ...
                {!!error && (
                  <Box mt={3}>
                    <Text color={Text.Color.Red3}>{error}</Text>
                  </Box>
                )}
              </Box>
            </Col>
  ...
  private onChangeDescription = (e: React.ChangeEvent<any>) => {
    this.props.rootStore.transactionFormStore.setDescription(e.target.value);
  };

  private onChangeDateFocused = ({ focused }: { focused: boolean | null }) => {
  ...
  private onSubmit = async (e: React.FormEvent) => {
    ...
    const {
      history,
      rootStore: {
        transactionFormStore: { isUpdatable, handleUpdate, handleCreate }
      }
    } = this.props;
    if (isUpdatable) {
      handleUpdate();
...
export const Transaction = inject("rootStore")(observer(TransactionPage));
