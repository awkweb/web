...
interface Props {
    ...
    categorizeTransactions: (budgetId: string) => void
...
export default class TransactionsStore implements Props {
    ...
    public addTransaction = (transaction: Transaction) => {
        this.transactions = [transaction, ...this.transactions].sort(dateSorter)
    }
        ...
        if (this.startDelete) {
            try {
                ...
                const deletedTransactions = this.transactions.filter(
                    t =>
                        transactionIds.includes(t.id) && get(() => t.budget.id),
                )
                this.transactions = [
                    ...this.transactions.filter(
    ...
    public categorizeTransactions = async (budgetId: string) => {
        try {
            const transactionIds = toJS(this.selectedTransactionIds)
